...
uint32_t CANBootWriteUpper(uint32_t id, uint8_t *pcBuf, uint8_t ui8Len)
{
    ...
    CAN1TxMessage.tx_efid = id;                   // 初始化发送帧ID
    CAN1TxMessage.tx_ft   = CAN_FT_DATA;          // 配置为数据帧
    CAN1TxMessage.tx_ff   = CAN_FF_EXTENDED;      // 配置为拓展帧
    memcpy(CAN1TxMessage.tx_data, pcBuf, ui8Len); // 复制发送数据
    CAN1TxMessage.tx_dlen = ui8Len;               // 配置数据长度
    if(can_message_transmit(CAN1, &CAN1TxMessage) == CAN_NOMAILBOX)
        return RETURN_ERROR; // 尝试发送但没有空闲邮箱
...
uint32_t CANBootGetcUpper(uint32_t *id, uint8_t *data_rx, uint8_t *ui8Len)
{
    ...
    if(Queue_CanMsgPop(&CAN1MsgQueue, &getMeg) == 0) // 尝试从接收队列取出数据
        return RETURN_ERROR;
    *id = getMeg.ID;
    memcpy(data_rx, getMeg.Data, getMeg.MsgLen);
    *ui8Len = getMeg.MsgLen;
